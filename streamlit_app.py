...
# ======================= Auto distance/step sync (no-crash) =======================
import re, numpy as np, pandas as pd, streamlit as st
from collections import Counter

def _marker_list(df: pd.DataFrame):
    marks = []
    for c in df.columns:
        if isinstance(c, str):
            m = re.match(r"^(\d+)_Time$", c.strip())
            if m:
                try:
                    marks.append(int(m.group(1)))
                except:
                    pass
    marks = sorted(set(marks), reverse=True)  # e.g. [1500,1400,1300,...]
    return marks

def _infer_step(marks):
    if len(marks) >= 2:
        diffs = [marks[i]-marks[i+1] for i in range(len(marks)-1)]
        diffs = [abs(d) for d in diffs if d > 0]
        if diffs:
            # choose the mode among reasonable split sizes
            cnt = Counter(diffs)
            step = max(cnt, key=lambda k: (cnt[k], k in (100,200), -k))
            # snap to {100,200} if close
            if abs(step-100) <= 10: return 100
            if abs(step-200) <= 20: return 200
            return step
    return 100  # safe default

def _infer_distance(marks, step_guess, current_input=None):
    """
    Heuristics:
    - Typical D = max_mark + step (e.g., 1400 race with 1300 top marker @100s)
    - Allow odd first-legs like 1250 (→ +50), 1160 (→ +160), 1100(+100), etc.
    - Prefer the candidate closest to the current input if present.
    """
    if not marks:
        return current_input or 1200

    top = marks[0]
    candidates = [
        top + step_guess,
        top + 50, top + 60, top + 80, top + 100, top + 120, top + 160
    ]
    # keep sensible range
    candidates = [int(c) for c in candidates if 600 <= c <= 3600]
    candidates = sorted(set(candidates))

    if current_input is not None:
        # pick the nearest to what the user typed
        best = min(candidates, key=lambda c: abs(c - float(current_input)))
    else:
        # default to top + step
        best = candidates[0]
    return best

def sync_distance_and_step(work_df: pd.DataFrame):
    marks = _marker_list(work_df)
    step_guess = _infer_step(marks)
    cur = None
    try:
        cur = float(st.session_state.get("race_distance_input", np.nan))
    except Exception:
        cur = None

    D_eff = _infer_distance(marks, step_guess, current_input=cur)

    # If different, update session_state and a global/local you use later
    changed = (cur is None) or (abs(float(D_eff) - float(cur)) > 0.01)
    if changed:
        st.session_state["race_distance_input"] = int(D_eff)

    # Also stash to attrs so all downstream code sees the same values
    try:
        metrics.attrs["STEP"] = step_guess
    except Exception:
        pass

    return int(D_eff), int(step_guess), changed

# ---- call it immediately after you have `work` from the upload ----
race_distance_input, STEP, _changed = sync_distance_and_step(work)
if _changed:
    st.toast(f"Distance auto-corrected to {race_distance_input} m (step {STEP}).", icon="✅")
# ======================= /Auto distance/step sync =======================
