...
if len(seg_markers)==0 and "Finish_Time" not in work.columns:
    ...
    for m in wanted:
        c=f"{m}_Time"
        if c in work.columns: segs.append((m+200,m,200.0,c))
    if "Finish_Time" in work.columns: segs.append((200,0,200.0,"Finish_Time"))
    if len(segs)==0:
        ...
        for i, (_, r) in enumerate(top8.iterrows()):
            row_times = work[work["Horse"]==r.get("Horse")].iloc[0] if ("Horse" in work.columns and "Horse" in metrics.columns and not work[work["Horse"]==r.get("Horse")].empty) else r
            y_vals=[]
            for (_,_,L,c) in segs:
