...
def normalize_headers_200m(df: pd.DataFrame, D_m: int) -> pd.DataFrame:
    """Normalize typical 200 m headers and drop duplicates (keep first)."""
    rename = {}
    for col in list(df.columns):
        ...
        # Generic "1600mTime" / "1600 Time" / "1600_split" → 1600_Time
...
# Convention:
#   X_Time  = time from (X+200) → X    (X ∈ {D-200, D-400, …, 200})
...
def build_metrics_200(df_in: pd.DataFrame, D_actual_m: float):
    ...
    # F200: first 200 m after the start = (D-200)
    ...
    # Grind: Finish only (handled via grind_speed_200)

    # --- convert to vs-field indices (100 = par) ---
    w["_F200_spd"]  = w.apply(
        lambda r: (200.0 / as_num(r.get(f200_col)))
        if f200_cols and pd.notna(as_num(r.get(f200_col))) and float(as_num(r.get(f200_col))) > 0 else np.nan,
        axis=1)
    ...
    for _, r in w.iterrows():
        ...
        EFF = 0.0 if (pd.isna(acc) or pd.isna(grd)) else \
              clamp(1.0 - (abs(acc-100.0) + abs(grd-100.0))/2.0/8.0, 0.0, 1.0)
...
def _repel_labels_builtin(ax, x, y, labels, *,
    ...
    for _ in range(max_iter):
        ...
        for t,(xi,yi) in zip(texts,xy):
            ...
            if abs(d-tgt)>1.0:
                v=(pt-pp)/(d+1e-9); pt2=pt+v*(0.6*(tgt-d))
...
if not needed_cols.issubset(metrics.columns):
    ...
    st.warning("Shape Map: required columns missing: " + miss)
else:
...
# ======================= Visual 2: Pace Curve — includes Finish =================
...
if len(seg_markers)==0 and "Finish_Time" not in work.columns:
    ...
    if len(segs) == 0:
        ...
        for (s,e,L,c) in segs: speed_df[c] = L / times_df[c]
...
PI_W_BARS = pi_weights_distance_and_context(float(race_distance_input),
                                            acc_med_for_bars, grd_med_for_bars)
...
if not top8_pi.empty:
    ...
    for key, label in [("F200_idx","F200"), ("tsSPI","tsSPI"), ("Accel","Accel"), ("Grind","Grind")]:
        ...
        ax3.bar(x, vals, bottom=bottoms, label=label, color=palette[key],
...
# 3) TFS (late variability vs mid pace) — last three pre-finish 200 m: 600, 400, 200
def tfs_row(r):
