# app.py
# Race Edge — v3.1 Unified + Hidden Abilities (complete app)

import io, os, re, math, json, time, sqlite3, datetime
import numpy as np
import pandas as pd
...
with st.sidebar:
    ...
    # --- Hidden Abilities: Going control ---
    st.markdown("### Going (today)")
    GOING = st.selectbox(
        "Surface condition (affects Hidden Abilities only)",
        ["Firm", "Good", "Good-to-Soft", "Soft", "Heavy"],
        index=1,
        help="Used to slightly re-weight late vs early ability in Hidden Abilities."
    )
...
def build_segment_plan_200(D: int) -> list[tuple[int, float]]:
    ...
    if rem > 0:
        first_end = D - rem
        if first_end >= 200: segs.append((first_end, float(rem)))
    else:
        first_end = D - 200
        if first_end >= 200: segs.append((first_end, 200.0))
    m = first_end - 200
    while m >= 200:
        segs.append((m, 200.0))
        m -= 200
...
def build_segment_plan_100(D: int) -> list[tuple[int, float]]:
    return [(m, 100.0) for m in range(D - 100, 99, -100)]
...
st.info(det_msg)
if DEBUG: st.write("Seg plan (end_marker, length_m):", seg_plan)
...
st.session_state["going_simple"] = GOING
...
def stage_columns_and_lengths(grid: str, D: int):
    ...
    if grid == "100m":
        ...
        grind_lens = []
        for c in grind_cols: grind_lens.append(100.0 if c == "100_Time" else float(finish_len_expected))
    else:
        if len(seg_plan) > 0:
...
def stage_speed_row(row, cols, lens):
    ...
    for c, L in zip(cols, lens):
        ...
        if pd.notna(t) and float(t) > 0:
            times.append(float(t)); dsum += float(L)
    if dsum <= 0 or len(times) == 0: return np.nan
...
def build_metrics_cached(_raw_hash: str, df: pd.DataFrame, D: int, grid: str, seg_plan: list, finish_len_expected: float):
    ...
    for _, r in w.iterrows():
        ...
        if pd.isna(acc) or pd.isna(grd): EFF = 0.0
        else:
...
def tfs_row(r):
    if grid == "100m":
        last_cols = [c for c in ["300_Time","200_Time","100_Time"] if c in work.columns]; unit = 100.0
    else:
        last_cols = [c for c in ["600_Time","400_Time","200_Time"] if c in work.columns]; unit = 200.0
    spds = []
    for c in last_cols:
...
h_med = float(np.nanmedian(hidden)); h_mad = float(np.nanmedian(np.abs(hidden - h_med)))
...
# ======================= Hidden Abilities v1 (separate model) =======================
def _mad_std_np(arr):
...
def _map_0_1(x, lo, hi):
    x = pd.to_numeric(x, errors="coerce")
    return np.where(pd.isna(x), 0.0, np.clip((x - lo) / (hi - lo), 0.0, 1.0))

def _ha_weights_for(distance_m: int, going: str):
    dm = float(distance_m or 1200)
    if dm <= 1100:   base = dict(ts=0.30, acc=0.32, grd=0.18, tfs=0.08, uei=0.07, asi=0.05)
    elif dm <= 1400: base = dict(ts=0.30, acc=0.28, grd=0.20, tfs=0.10, uei=0.07, asi=0.05)
    elif dm <= 1800: base = dict(ts=0.28, acc=0.24, grd=0.24, tfs=0.12, uei=0.07, asi=0.05)
    else:            base = dict(ts=0.26, acc=0.20, grd=0.28, tfs=0.14, uei=0.07, asi=0.05)
    tilt = {"Firm":(-0.02,+0.02), "Good":(0.00,0.00), "Good-to-Soft":(-0.01,+0.01),
            "Soft":(-0.02,+0.02), "Heavy":(-0.03,+0.03)}
    acc_delta, grd_delta = tilt.get(going, (0.0, 0.0))
    base["acc"] = max(0.05, base["acc"] + acc_delta)
    base["grd"] = max(0.05, base["grd"] + grd_delta)
    s = sum(base.values())
    return {k: v / s for k, v in base.items()}

def build_hidden_abilities(metrics_df: pd.DataFrame, hh_df: pd.DataFrame | None,
                           distance_m: int, going: str) -> pd.DataFrame:
    df = metrics_df.copy()
    C_ts   = _map_0_1(df["tsSPI"], 100.0, 106.0)
    acc = pd.to_numeric(df["Accel"], errors="coerce")
    ts  = pd.to_numeric(df["tsSPI"], errors="coerce")
    C_acc_latent = np.where((ts >= 101.0) & np.isfinite(acc), _map_0_1(102.0 - acc, 0.0, 6.0), 0.0)
    C_grd  = _map_0_1(df["Grind"], 100.0, 106.0)
    if hh_df is not None and "TFS_plus" in hh_df.columns:
        tfs_plus = pd.to_numeric(hh_df["TFS_plus"], errors="coerce").fillna(0.0)
        C_tfs = np.clip(tfs_plus / 0.6, 0.0, 1.0)
    else:
        C_tfs = np.zeros(len(df))
    if hh_df is not None and "UEI" in hh_df.columns:
        uei = pd.to_numeric(hh_df["UEI"], errors="coerce").fillna(0.0)
        C_uei = np.clip(uei / 0.6, 0.0, 1.0)
    else:
        C_uei = np.zeros(len(df))
    if hh_df is not None and "ASI2" in hh_df.columns:
        C_asi = np.clip(pd.to_numeric(hh_df["ASI2"], errors="coerce").fillna(0.0), 0.0, 1.0)
    else:
        C_asi = np.zeros(len(df))
    W = _ha_weights_for(int(distance_m), str(going or "Good"))
    raw01 = (W["ts"]*C_ts + W["acc"]*C_acc_latent + W["grd"]*C_grd + W["tfs"]*C_tfs + W["uei"]*C_uei + W["asi"]*C_asi)
    raw = pd.to_numeric(raw01, errors="coerce")
    med = float(np.nanmedian(raw)) if np.isfinite(np.nanmedian(raw)) else 0.0
    idx_raw = 100.0 * (raw / (med if med > 0 else 1.0))
    x = idx_raw[np.isfinite(idx_raw)].values
    if x.size == 0:
        center = 100.0
    else:
        med_race = float(np.median(x))
        ...
        center = alpha * med_race + (1 - alpha) * 100.0
    HA_idx = 100.0 * (raw / ((center / 100.0) * (med if med > 0 else 1.0)))
    deltas = HA_idx - 100.0
    sigma = _mad_std_np(deltas)
    if np.isnan(sigma) or sigma < 1.2:
        factor = 1.2 / (sigma if (sigma and sigma > 0) else 1.2)
        HA_idx = 100.0 + (deltas * min(factor, 1.25))
    z_sigma = _mad_std_np(raw)
    if np.isnan(z_sigma) or z_sigma <= 0: z = np.zeros(len(raw))
    else: z = (raw - med) / z_sigma
    HA_std = np.clip(5.0 + 2.1 * z, 0.0, 10.0)
    out = pd.DataFrame({
        "Horse": df.get("Horse", pd.Series([None]*len(df))),
        "Finish_Pos": pd.to_numeric(df.get("Finish_Pos"), errors="coerce"),
        "HA_idx": np.round(HA_idx, 3),
        "HA_std": np.round(HA_std, 3),
        "HA_ts": np.round(C_ts, 3),
        "HA_acc_latent": np.round(C_acc_latent, 3),
        "HA_grd": np.round(C_grd, 3),
        "HA_tfs": np.round(C_tfs, 3),
        "HA_uei": np.round(C_uei, 3),
        "HA_asi": np.round(C_asi, 3),
    })
    return out

HA = build_hidden_abilities(metrics, st.session_state.get("hh", None), D, st.session_state.get("going_simple", "Good"))
st.session_state["ha"] = HA

# ======================= Minimal on-screen check =======================
...
st.caption("Batch 2 computed stage indices → PI v3.1 → GCI and Hidden Horses v2 (grid-aware). HA computed separately.")
...
def _repel_labels_builtin(ax, x, y, labels, *, init_shift=0.18, k_repel=0.012, max_iter=250):
...
if not need_cols.issubset(metrics.columns):
    ...
    if dfm.empty:
        ...
        st.caption("Each bubble is a runner. Size = PI. X: Accel vs field; Y: Grind vs field. Colour = tsSPI vs field.")
...
if len(segs) == 0:
    ...
    if "Finish_Pos" in metrics.columns and metrics["Finish_Pos"].notna().any():
        top8 = metrics.sort_values("Finish_Pos").head(8); top8_rule = "Top-8 by Finish_Pos"
    else:
        top8 = metrics.sort_values("PI", ascending=False).head(8); top8_rule = "Top-8 by PI"
    x_idx = list(range(len(segs))); x_labels = [lab for (lab,_,_) in segs]
    ...
    st.caption(f"Top-8 plotted: {top8_rule}. Finish segment included.")

# ---- Visual 3: Top-8 PI stacks
...
if top8_pi.empty:
    st.info("No PI values available to plot.")
else:
...
if hh is None or hh.empty:
    ...
    view = hh.copy()
    if show_flagged_only: view = view[view["Tier"] != ""]
    for c in ["PI","GCI","tsSPI","Accel","Grind","SOS","ASI2","TFS","UEI","HiddenScore"]:
        if c in view.columns:
            view[c] = pd.to_numeric(view[c], errors="coerce").map(lambda x: "" if pd.isna(x) else f"{x:.3f}")
    view = view.sort_values(by=["_tier_rank","HiddenScore","PI","Finish_Pos"], ascending=[True, False, False, True], kind="mergesort").drop(columns=["_tier_rank"])
    st.dataframe(view[cols_hh], use_container_width=True)
    st.caption("Flag rules: 🔥 Top Hidden ≥ 1.8 • 🟡 Notable Hidden ≥ 1.2.")

# ---- Hidden Abilities display (separate)
st.markdown("## 🧬 Hidden Abilities (separate model)")
ha = st.session_state.get("ha", None)
if ha is None or ha.empty:
    st.info("Hidden Abilities not available yet.")
else:
    view = ha.copy()
    cols_ha = ["Horse","Finish_Pos","HA_idx","HA_std","HA_ts","HA_acc_latent","HA_grd","HA_tfs","HA_uei","HA_asi"]
    for c in cols_ha:
        if c not in view.columns: view[c] = np.nan
    def _tags_row(r):
        t = []
        if pd.to_numeric(r.get("HA_grd"), errors="coerce") >= 0.6: t.append("late-strong")
        if pd.to_numeric(r.get("HA_acc_latent"), errors="coerce") >= 0.5: t.append("early-upside")
        if pd.to_numeric(r.get("HA_tfs"), errors="coerce") >= 0.4: t.append("trip-friction")
        if pd.to_numeric(r.get("HA_asi"), errors="coerce") >= 0.5: t.append("bias-hardy")
        if pd.to_numeric(r.get("HA_ts"), errors="coerce") >= 0.6: t.append("good-cruise")
        return ", ".join(t)
    view["Tags"] = view.apply(_tags_row, axis=1)
    view = view.sort_values(by=["HA_std","Finish_Pos"], ascending=[False, True])
    for c in ["HA_idx","HA_std","HA_ts","HA_acc_latent","HA_grd","HA_tfs","HA_uei","HA_asi"]:
        view[c] = pd.to_numeric(view[c], errors="coerce").map(lambda x: "" if pd.isna(x) else f"{x:.3f}")
    st.dataframe(view[["Horse","Finish_Pos","HA_idx","HA_std","Tags","HA_ts","HA_acc_latent","HA_grd","HA_tfs","HA_uei","HA_asi"]],
                 use_container_width=True)
    st.caption("Dual scale: HA_idx = field index (100=median). HA_std = cross-race 0–10. Tags summarize component strengths.")

# ======================= Batch 4 — Database Integration (SQLite) =======================
with st.sidebar:
    st.markdown("### Database")
    db_path = st.text_input("SQLite DB file", value="race_edge.sqlite")
    if not db_path.strip(): st.stop()
...
-- Hidden Abilities (separate)
CREATE TABLE IF NOT EXISTS hidden_abilities (
  id          INTEGER PRIMARY KEY AUTOINCREMENT,
  run_id      INTEGER NOT NULL REFERENCES runs(id) ON DELETE CASCADE,
  ha_idx      REAL,
  ha_std      REAL,
...
CREATE INDEX IF NOT EXISTS idx_ha_run ON hidden_abilities(run_id);
"""
...
def upsert_horse(conn: sqlite3.Connection, name: str, country: str|None=None, yob: int|None=None) -> int:
    ...
    if not can:
        can = f"unknown-{int(_now_ts()*1000)}"; name = name if str(name).strip() else "Unknown"
    cur = conn.cursor()
    cur.execute("SELECT id FROM horses WHERE canonical = ?", (can,))
    row = cur.fetchone()
    if row: return int(row[0])
    cur.execute("INSERT INTO horses(name, country, yob, canonical, created_ts) VALUES (?,?,?,?,?)",
...
def upsert_race(conn: sqlite3.Connection, *, meeting_date: str|None, venue: str|None,
    ...
    cur.execute("""SELECT id FROM races
                   WHERE meeting_date IS ? AND venue IS ? AND race_num IS ? AND distance_m = ? AND grid = ?""",
    ...
    cur.execute("""INSERT INTO races(venue, meeting_date, race_num, distance_m, surface, going, class, grid, meta_json, created_ts)
                ...
                (venue, meeting_date, race_num, int(distance_m),
                 ...
                 grid, json.dumps(meta or {}, ensure_ascii=False), _now_ts()))
    conn.commit()
    return int(cur.lastrowid)
...
def insert_run(conn: sqlite3.Connection, *, race_id: int, horse_id: int, row: pd.Series) -> int:
    ...
    cur.execute(
        """INSERT INTO runs(race_id, horse_id, draw, jockey, trainer, weight_carried, finish_pos,
           ...
           VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
        (race_id, horse_id, row.get("Draw"), row.get("Jockey"), row.get("Trainer"),
         _safe_float(row.get("WeightCarried")), _safe_int(row.get("Finish_Pos")),
         _safe_float(row.get("RaceTime_s")), _safe_float(row.get("F200_idx")), _safe_float(row.get("tsSPI")),
         _safe_float(row.get("Accel")), _safe_float(row.get("Grind")), _safe_float(row.get("PI")),
         _safe_float(row.get("GCI")), _now_ts()))
    conn.commit()
    return int(cur.lastrowid)

def insert_splits_for_run(conn: sqlite3.Connection, *, run_id: int, grid: str,
                          seg_plan: list[tuple[int, float]], row_times: pd.Series, D: int, finish_len_expected: float):
    cur = conn.cursor(); created = _now_ts()
    if grid == "200m":
        for end_m, L in seg_plan:
            col = f"{end_m}_Time"; t = _safe_float(row_times.get(col))
            cur.execute(
        ...
        cur.execute(
            "INSERT INTO splits(run_id, marker_end_m, length_m, time_s, col_name, created_ts) VALUES (?,?,?,?,?,?)",
            (run_id, 0, float(finish_len_expected), tfin, "Finish_Time", created)
        )
    else:
        for m in range(int(D) - 100, 99, -100):
            col = f"{m}_Time"; t = _safe_float(row_times.get(col))
            cur.execute(
                "INSERT INTO splits(run_id, marker_end_m, length_m, time_s, col_name, created_ts) VALUES (?,?,?,?,?,?)",
...
def insert_shape_point(conn: sqlite3.Connection, *, run_id: int, accel: float|None, grind: float|None, tsspi: float|None):
    ...
    if a is None or g is None or t is None: return
    cur = conn.cursor()
    cur.execute(
...
def insert_hidden_abilities(conn: sqlite3.Connection, *, run_id: int, ha_row: pd.Series):
    cur = conn.cursor()
    cur.execute(
        """INSERT INTO hidden_abilities(run_id, ha_idx, ha_std, c_ts, c_acc_lat, c_grd, c_tfs, c_uei, c_asi, notes, created_ts)
           VALUES (?,?,?,?,?,?,?,?,?,?,?)""",
        (int(run_id), _safe_float(ha_row.get("HA_idx")), _safe_float(ha_row.get("HA_std")),
         _safe_float(ha_row.get("HA_ts")), _safe_float(ha_row.get("HA_acc_latent")), _safe_float(ha_row.get("HA_grd")),
         _safe_float(ha_row.get("HA_tfs")), _safe_float(ha_row.get("HA_uei")), _safe_float(ha_row.get("HA_asi")),
         None, _now_ts())
    )
    conn.commit()
...
with colA:
    ...
    mdate   = st.text_input("Meeting date (YYYY-MM-DD)", value="")
with colB:
    race_no = st.number_input("Race number", min_value=1, max_value=20, value=1)
    surface = st.text_input("Surface (opt.)", value="")
going   = st.text_input("Going (opt.)", value="")
rclass  = st.text_input("Class (opt.)", value="")
meta_preview = {"surface": surface or None, "going": going or None, "class": rclass or None}
if not (going or "").strip():
    meta_preview["going"] = st.session_state.get("going_simple", "Good")

save_clicked = st.button("Save race, runners, sectionals, shape points & Hidden Abilities")

if save_clicked:
    try:
        init_db(db_path)
        conn = _connect(db_path)
        race_id = upsert_race(conn, meeting_date=mdate or None, venue=venue or None,
                              race_num=int(race_no) if race_no else None,
                              distance_m=int(D), grid=grid, meta=meta_preview)
        work_by_horse = work.set_index("Horse", drop=False) if "Horse" in work.columns else None
        ha_df = st.session_state.get("ha", None)
        ...
        for _, r in metrics.iterrows():
            ...
            if work_by_horse is not None and horse_name in work_by_horse.index:
                ...
                row_times = r
            insert_splits_for_run(conn, run_id=run_id, grid=grid, seg_plan=seg_plan, row_times=row_times, D=D, finish_len_expected=finish_len_expected)
            insert_shape_point(conn, run_id=run_id, accel=r.get("Accel"), grind=r.get("Grind"), tsspi=r.get("tsSPI"))

            # Hidden Abilities row
            if ha_df is not None and "Horse" in ha_df.columns:
                _match = ha_df[ha_df["Horse"].astype(str).str.strip() == horse_name]
                if not _match.empty:
                    insert_hidden_abilities(conn, run_id=run_id, ha_row=_match.iloc[0])
...
if browse_ok and not horses_df.empty:
        ...
        try:
            ...
            if hist.empty:
                ...
                if segs_tbl.empty:
                    ...
                    segs_tbl["speed_mps"] = segs_tbl.apply(lambda r: (float(r["length_m"])/float(r["time_s"])) if (pd.notna(r["time_s"]) and r["time_s"]>0) else np.nan, axis=1)
                    segs_view = segs_tbl.copy()
                ...
                # Hidden Abilities preview for most recent run
                try:
                    ha_preview = pd.read_sql_query(
                        "SELECT ha_idx, ha_std, c_ts, c_acc_lat, c_grd, c_tfs, c_uei, c_asi FROM hidden_abilities WHERE run_id = ?",
                        conn, params=(last_run_id,)
                    )
                    if not ha_preview.empty:
                        st.markdown("**Most recent run — Hidden Abilities**")
                        show_ha = ha_preview.copy()
                        for c in show_ha.columns:
                            show_ha[c] = pd.to_numeric(show_ha[c], errors="coerce").map(lambda x: "" if pd.isna(x) else f"{x:.3f}")
                        st.dataframe(show_ha, use_container_width=True)
                except Exception:
                    pass
    ...
    if browse_ok: st.info("No horses yet — save a race first.")

# ======================= Batch 5 — Reports (Race + Horse PDF) =======================
with st.sidebar:
    st.markdown("### Reports & Profiles")
    db_path = st.text_input("SQLite DB file (same as above)", value=db_path)
...
def _shape_scatter(img_width=(7.6, 6.0), *, df_points: pd.DataFrame, title: str):
    d = df_points.copy()
    for c in ["accel_delta","grind_delta","tsspi_delta"]: d[c] = pd.to_numeric(d[c], errors="coerce")
    d = d.dropna(subset=["accel_delta","grind_delta","tsspi_delta"])
    if d.empty: return None, None
    xv = d["accel_delta"].to_numpy(); yv = d["grind_delta"].to_numpy(); cv = d["tsspi_delta"].to_numpy()
    ...
    if len(labels) <= 18:
        for xi, yi, lab in zip(xv, yv, labels):
            if lab and lab != "None":
                ax.text(xi, yi, lab, fontsize=8.2, bbox=dict(boxstyle="round,pad=0.18", fc="white", ec="none", alpha=0.70))
    ax.set_xlim(-lim, lim); ax.set_ylim(-lim, lim)
...
def _pdf_safe_number(x, ndigits=3):
    try:
        ...
        fmt = f"{{:.{ndigits}f}}"; return fmt.format(v)
    except: return ""
...
def make_race_pdf(*, dbfile: str, race_id: int, shape_png: bytes|None=None):
    ...
    conn.close()
    if race.empty:
        st.error("Race not found."); return None
    R = race.iloc[0]
    hdr_bits = [
    ...
    if not runs.empty:
        ...
        data = [list(["Horse","Finish_Pos","RaceTime_s","F200_idx","tsSPI","Accel","Grind","PI","GCI"])] + tab.fillna("").astype(str).values.tolist()
    ...
    story.append(Paragraph(
        ...
        "GCI is a 0–10 composite of time merit, late quality, mid-race strength, and efficiency.", P))
    doc.build(story)
    buf.seek(0)
    return buf
...
def make_horse_pdf(*, horse_name: str, hist_table: pd.DataFrame, shape_png: bytes|None):
    ...
    if not hist_table.empty:
        ...
        for c in ["race_time_s","f200_idx","tsspi","accel","grind","pi","gci"]:
            if c in tab.columns:
                tab[c] = tab[c].map(lambda x: "" if pd.isna(x) else (_pdf_safe_number(x, 2) if c!="race_time_s" else _pdf_safe_number(x, 2)))
        cols = [c for c in ["meeting_date","venue","race_num","distance_m","grid","finish_pos","race_time_s","f200_idx","tsspi","accel","grind","pi","gci"] if c in tab.columns]
    ...
    if shape_png:
        ...
        story.append(Spacer(0, 8))
    story.append(Paragraph(
        "Each point represents a run (size = PI where available). Axes show deviation vs field in points (100=par). Colour is tsSPIΔ.", P))
...
try:
    ...
    races = pd.read_sql_query("""
        SELECT id, COALESCE(meeting_date,'????-??-??') as meeting_date,
               COALESCE(venue,'?') as venue, COALESCE(race_num, '?') as race_num,
               distance_m, grid
        FROM races
        ORDER BY COALESCE(meeting_date,'9999-12-31') DESC, venue COLLATE NOCASE ASC, race_num ASC
    """, conn); conn.close(); race_picker_ok=True
except Exception: st.info("Could not open DB. Check path.")
if race_picker_ok and not races.empty:
    ...
    if choice and choice != "— pick —":
        ...
        pts = pd.read_sql_query("""
            SELECT h.name as label, (ru.accel-100.0) as accel_delta, (ru.grind-100.0) as grind_delta,
                   (ru.tsspi-100.0) as tsspi_delta, ru.pi
            FROM runs ru JOIN horses h ON h.id = ru.horse_id
            WHERE ru.race_id = ?
        """, conn, params=(rid,)); conn.close()
...
try:
    conn = _connect(db_path)
    horses_df = pd.read_sql_query("SELECT id, name FROM horses ORDER BY name COLLATE NOCASE", conn)
    conn.close(); horse_picker_ok = True
except Exception:
    st.info("Could not open DB. Check path.")
if horse_picker_ok and not horses_df.empty:
    ...
    if pick and pick != "— pick —":
        ...
        hist = pd.read_sql_query("""
            SELECT r.id as race_id, r.meeting_date, r.venue, r.race_num, r.distance_m, r.grid,
                   ru.finish_pos, ru.race_time_s, ru.f200_idx, ru.tsspi, ru.accel, ru.grind, ru.pi, ru.gci,
                   ru.id as run_id
            FROM runs ru JOIN races r ON r.id = ru.race_id
            WHERE ru.horse_id = ?
        ...
        sp = pd.read_sql_query("""
            ...
            FROM shape_points sp JOIN runs ru ON ru.id = sp.run_id
            WHERE ru.horse_id = ?
        """, conn, params=(hid,))
        st.markdown(f"### History for **{pick}**")
        if hist.empty:
...
# ======================= Batch 6 — Stability & Footer =======================
st.markdown("---")
st.markdown("### 🧭 Race Edge — v3.1 Unified + Hidden Abilities")
st.caption(
    "Handles both 100m and 200m splits • Distance + Context PI v3.1 • Hidden Horses v2 • "
    "Hidden Abilities (separate) • Database & Reports • Optimized caching."
)

# Diagnostics (optional lightweight)
if "runtime_stats" not in st.session_state: st.session_state["runtime_stats"] = []
...
with st.sidebar.expander("🩺 Diagnostics"):
    ...
    if st.session_state["runtime_stats"]:
        for line in reversed(st.session_state["runtime_stats"]): st.text(line)
    else:
        st.caption("No events logged yet.")
    if st.button("Clear logs"):
        st.session_state["runtime_stats"] = []; st.success("Runtime logs cleared.")
st.markdown("---")
st.markdown("© 2025 Race Edge Analytics — All local processing. Unified build with HA.")
